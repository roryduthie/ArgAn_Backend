...
def make_treemap(dataframe, path, width, height):
...
def object_vis(ids):
    ...
    event_df = event_df.sort_values(by='ID', ascending=True)

    actor_df = event_df[['Object']]

    actor_df['Object'] = actor_df['Object'].str.strip()
    actor_df['Object'] = actor_df['Object'].str.lower()

    actors = actor_df['Object'].value_counts().to_frame().reset_index()
    actors.columns = ['Object','Count']
...
def actor_vis(ids):
    ...
    actor_df = event_df[['Agent']]

    actor_df['Agent'] = actor_df['Agent'].str.strip()
    actor_df['Agent'] = actor_df['Agent'].str.lower()

    actors = actor_df['Agent'].value_counts().to_frame().reset_index()
...
def actor_vis_view(ids):
    ...
    data_dict = actors.to_dict(orient='records')


    return render_template('display_graph.html',
                               div_placeholder=Markup(dv)
                              )
def get_location(text, nlp):
    locations = []
    doc = nlp(text)
    for ent in doc.ents:
        if ent.label_ == 'GPE':
...
def get_lat_lon(locations):
    ...
    for location in locations:
        ...
        l_l = get_boundingbox_country(country=location,output_as='center')
        if len(l_l) > 1:
...
def event_location_vis_view(ids):
    ...
    if event_df.empty:
        ...
        space = event_df['Space'].tolist()
        place = event_df['Place'].tolist()
        space = [x for x in space if x]
        place = [x for x in place if x]

    text_list.extend(space)
    text_list.extend(place)

    nlp = spacy.load("en")

    for text in text_list:
        locations = get_location(text, nlp)
        if len(locations) > 0:
                location_node.extend(locations)
    if len(location_node) < 1:
        i_node_flag = True

    if i_node_flag:

        i_nodes = centra.get_i_node_list(graph)
        i_node_df = pd.DataFrame(i_nodes, columns=['ID', 'text'])
        i_text = i_node_df['text'].tolist()
        text_list.extend(i_text)
        for text in text_list:
            locations = get_location(text, nlp)
            if len(locations) > 0:
                location_node.extend(locations)
    ...
    if len(location_node) < 1:
        ...
        name_lat_lons = get_lat_lon(location_node)

        if len(name_lat_lons) < 1:
            lon_list = []
            lat_list = []
            dv = make_map_plot(lat_list, lon_list, [])
            return render_template('display_graph.html',
            ...
            lat_df = pd.DataFrame(name_lat_lons, columns=['Location', 'Lat', 'Lon'])
            lat_list = lat_df['Lat']
            lon_list = lat_df['Lon']
            txt_list = lat_df['Location']
            dv = make_map_plot(lat_list, lon_list, txt_list)
...
def hevy_event_vis_view(ids):
    ...
    #dv = make_scatter_plot(event_df, 'ID', 'Value', 'ID', None, ['Name'])
    #data_dict = event_df.to_dict(orient='records')

    #dict_list = []
    #for data in data_dict:
    #    data_string = ' '.join('{0}: {1}'.format(key, val) for key, val in data.items())
    #    dict_list.append(data_string)
    #value_list = list(range(1,len(dict_list)))



    #value_list.sort(reverse=True)

    #height = 100 * len(dict_list)
    #dv = make_funnel_plot(dict_list, value_list, 2000)
    ...
    for node in nodes:
        ...
        if ntype == 'Event':
            ...
            try:
                ...
                circa = ''
            try:
                atplace = node['atPlace']

            except:
                atplace = ''
            event_list.append((ID, event_name, involved_agent, involved_object, inSpace, atplace,circa))
    return event_list
...
def interaction_vis_view(ids):
    ...
    df_interactions[['speaker1', 'speaker2']] = np.sort(df_interactions[['speaker1', 'speaker2']], axis=1)
    inters = df_interactions.groupby(['speaker1','speaker2'])['interaction'].agg('sum').to_frame().reset_index()
    df_interactions['speaker1'] = df_interactions['speaker1'].astype(str)
    df_interactions['speaker2'] = df_interactions['speaker2'].astype(str)


    html = make_data_table(inters)
...
def make_data_table(dataframe):


    cols = list(dataframe.columns)
    cell_vals = []
    for col in cols:
        cell_vals.append(list(dataframe[col]))
...
def make_funnel_plot(text_list, value_list, height):
...
def make_scatter_plot(df, x, y, color, size, hover_data):
    fig = px.scatter(df, x=x, y=y, color=color,
                 size=size, hover_data=hover_data)
    dv = plotly.io.to_html(fig, include_plotlyjs=False, full_html=False)
    return dv

def make_map_plot(lat, lon, text ):
